...
namespace AlphaOmega.Debug.Native
{
	...
	{
		/// <summary>The ATA_PASS_THROUGH_EX structure is used in conjunction with an <see cref="Constant.IOCTL_ATA.PASS_THROUGH"/> request to instruct the port driver to send an embedded ATA command to the target device</summary>
		...
		public struct ATA_PASS_THROUGH_EX
		{
			...
			public enum AtaPassThroughFlags : UInt16
			{
				...
				/// <remarks>When this flag is set, the contents of the PreviousTaskFile member in the <see cref="ATA_PASS_THROUGH_EX"/> structure should be valid</remarks>
				_48BIT_COMMAND = 1 << 3,
			...
			/// <summary>Specifies the length in bytes of the <see cref="ATA_PASS_THROUGH_EX"/> structure</summary>
			...
			/// <summary>Indicates the size, in bytes, of the data buffer</summary>
			/// <remarks>If an underrun occurs, the miniport driver must update this member to the number of bytes that were actually transferred</remarks>
